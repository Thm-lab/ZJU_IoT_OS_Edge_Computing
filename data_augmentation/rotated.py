...
def getRotatedAnno(Pi_angle,a,b,anno_path,anno_write_path):
    ...
    x_max = float(size.find('width').text)
    y_max = float(size.find('height').text)
    for obj in objects:
        ...
        if X_MIN<0 or X_MAX>x_max or Y_MIN<0 or Y_MAX >y_max:
...
img_dir='../backup/images'
anno_dir='../backup/xml_annotations'
img_write_dir='../datasets/ro_images'
anno_write_dir='../datasets/xml_ro_annotations'
